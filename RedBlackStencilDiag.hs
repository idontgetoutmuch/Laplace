...
relaxStep :: Double
             ...
             -> Stencil DIM2 Double
             -> Array D DIM2 Double
relaxStep !omega !arrOld !arrNbs !boundValue !boundMask !stencil1 !stencil2
  = altRows (f stencil1) (f stencil2)
  where
    {-# INLINE f #-}
    f s = A.szipWith (+) boundValue
          $ A.szipWith (*) boundMask
          $ A.szipWith (weightedSum omega) arrOld
          $ A.smap (/4)
...
weightedSum !omega !old !new = (1-omega)*old + omega*new
